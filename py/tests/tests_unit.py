...
class FrameTransformationsTest(unittest.TestCase):
    ...
    def test_h2o_frame_2_data_frame_new(self):
        hc = self._hc
        h2o_frame = h2o.upload_file(unit_test_utils.locate("smalldata/prostate/prostate.csv"))
...
class H2OConfTest(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls._spark = SparkSession.builder.config(conf = generic_test_utils.get_default_spark_conf().set("spark.ext.h2o.cloud.name", "test-cloud")).getOrCreate()
        unit_test_utils.set_up_class(cls)
